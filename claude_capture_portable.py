...
class ConfiguredLogger:
    ...
    def _setup_logger(self):
        ...
        self.logger.set_min_level(level_map.get(self.logging_config.get("logLevel", "INFO"), LogLevel.INFO))
        ...
        self.logger.add_handler(FilteredFileHandler(str(log_file), format_func, self.logging_config))
...
class ClaudeCaptureRunner:
    def __init__(self, prompt_library_path=None, roles_config_path=None):
        ...
        self.prompt_library_path = Path(prompt_library_path) if prompt_library_path else SCRIPT_DIR / "prompt_library.xml"
        self.roles_config_path = Path(roles_config_path) if roles_config_path else SCRIPT_DIR / "roles_config.json"
        self.prompts = {}
        self.roles_config = {}
    ...
    def _find_claude_command(self):
        """Find the Claude command in various locations"""
    ...
    def load_prompt_library(self):
        ...
        self.logger.log(LogLevel.INFO, "CONFIGURATION", "PARSERS", "load_prompt_library",
                       "Loading XML prompt library", path=str(self.prompt_library_path))
        try:
            ...
            for role_elem in roles.findall('prompt'):
                ...
                self.logger.log(LogLevel.DEBUG, "CONFIGURATION", "PARSERS", "load_prompt_library",
                               f"Parsed role: {role_key}", levels_found=level_count)
                        
            self.logger.log(LogLevel.INFO, "CONFIGURATION", "PARSERS", "load_prompt_library",
                           "Successfully loaded prompt library", total_roles=role_count)
                        
        except Exception as e:
            self.logger.log(LogLevel.ERROR, "ERROR_HANDLING", "PARSERS", "load_prompt_library",
                           f"Error loading prompt library: {e}", exception_type=type(e).__name__)
            print(f"Error loading prompt library: {e}")
            sys.exit(1)
            
    def load_roles_config(self):
        """Load the JSON roles configuration."""
        self.logger.log(LogLevel.INFO, "CONFIGURATION", "PARSERS", "load_roles_config",
                       "Loading roles configuration", path=str(self.roles_config_path))
        try:
            with open(self.roles_config_path, 'r') as f:
                data = json.load(f)
                self.roles_config = data.get('roles', {})
                
            enabled_roles = sum(1 for role, levels in self.roles_config.items() if any(levels.values()))
            self.logger.log(LogLevel.INFO, "CONFIGURATION", "PARSERS", "load_roles_config",
            ...
            self.logger.log(LogLevel.ERROR, "ERROR_HANDLING", "PARSERS", "load_roles_config",
                           f"Error loading roles config: {e}", exception_type=type(e).__name__)
            print(f"Error loading roles config: {e}")
            sys.exit(1)
            
    def get_enabled_role_prompts(self):
        """Get all enabled role prompts based on configuration."""
        self.logger.log(LogLevel.INFO, "CODE_ANALYSIS", "ANALYZERS", "get_enabled_role_prompts",
        ...
        for role_name, levels in self.roles_config.items():
            ...
            if any(levels.values()):
                ...
                if role_name in self.prompts:
                    for level in ['critical', 'standard', 'best_practice']:
                        if levels.get(level, False) and level in self.prompts[role_name]:
                    ...
                    if combined_prompt:
                        ...
                        self.logger.log(LogLevel.DEBUG, "CODE_ANALYSIS", "ANALYZERS", 
    ...
    def handle_timeout(self, process):
        ...
        self.logger.log(LogLevel.WARNING, "MONITORING", "HANDLERS", "handle_timeout",
                       "Timeout reached for Claude process", process_pid=process.pid)
        if pyautogui:
            try:
                self.logger.log(LogLevel.INFO, "MONITORING", "HANDLERS", "handle_timeout",
                ...
                self.logger.log(LogLevel.ERROR, "ERROR_HANDLING", "HANDLERS", "handle_timeout",
    ...
    def run_claude_with_prompt(self, combined_prompt, role_name):
        ...
        self.logger.log(LogLevel.INFO, "CODE_GENERATION", "SERVICES", "run_claude_with_prompt",
                       f"Starting Claude Code for role: {role_name}",
                       prompt_length=len(combined_prompt))
        
        # Use the capture_claude_print function approach
        cmd = [self.claude_path, '--print', combined_prompt]
        
        self.logger.log(LogLevel.DEBUG, "CODE_GENERATION", "SERVICES", "run_claude_with_prompt",
                       "Executing Claude command", command=' '.join(cmd[:2] + ['<prompt>']))
        ...
        try:
            ...
            self.logger.log(LogLevel.DEBUG, "MONITORING", "SERVICES", "run_claude_with_prompt",
                           "Claude process started", pid=process.pid)
            ...
            if process.returncode != 0:
                self.logger.log(LogLevel.ERROR, "ERROR_HANDLING", "SERVICES", "run_claude_with_prompt",
                               f"Claude returned non-zero code for role: {role_name}",
                               return_code=process.returncode,
                               stderr=stderr[:500])  # Log first 500 chars of error
    ...
    def init_log_file(self, user_prompt):
        ...
        self.logger.log(LogLevel.INFO, "FILE_OPERATIONS", "SERVICES", "init_log_file",
                       "Initializing capture log", filename=log_filename)
        ...
        try:
            ...
            self.logger.log(LogLevel.INFO, "FILE_OPERATIONS", "SERVICES", "init_log_file",
                           "Successfully initialized capture log", filename=log_filename)
            return log_filename
            
        except Exception as e:
            self.logger.log(LogLevel.ERROR, "ERROR_HANDLING", "SERVICES", "init_log_file",
    ...
    def append_result_to_log(self, result, log_filename):
        ...
        self.logger.log(LogLevel.INFO, "FILE_OPERATIONS", "SERVICES", "append_result_to_log",
                       "Appending result to log", filename=log_filename, role=result['role'])
        ...
        try:
            ...
            self.logger.log(LogLevel.INFO, "FILE_OPERATIONS", "SERVICES", "append_result_to_log",
                           "Successfully appended result to log", filename=log_filename)
            
        except Exception as e:
            self.logger.log(LogLevel.ERROR, "ERROR_HANDLING", "SERVICES", "append_result_to_log",
    ...
    def finalize_log(self, results, log_filename):
        ...
        try:
            ...
            self.logger.log(LogLevel.INFO, "FILE_OPERATIONS", "SERVICES", "finalize_log",
            ...
            self.logger.log(LogLevel.ERROR, "ERROR_HANDLING", "SERVICES", "finalize_log",
    ...
    def run(self, user_prompt):
        ...
        self.logger.log(LogLevel.INFO, "PROJECT_MANAGEMENT", "CONTROLLERS", "run",
                       "Starting Claude capture run", user_prompt_length=len(user_prompt))
        ...
        if not enabled_prompts:
            self.logger.log(LogLevel.WARNING, "CONFIGURATION", "CONTROLLERS", "run",
        ...
        for idx, role_info in enumerate(enabled_prompts, 1):
            ...
            if result['return_code'] == 0:
                print(f"\n✅ Response received and saved for {role_info['role']}")
            else:
                print(f"\n❌ Error occurred for {role_info['role']}: {result['error']}")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print('  python claude_capture_portable.py "Implement a new authentication system"')
        sys.exit(1)
